"""High-resolution favicon comparison script.

This script is based on `tests/test_favicons.py` but changes the goal:
- Fetch the *highest quality* favicon per domain.
- PNG and SVG are allowed (SVG is preferred).
- Save every fetched favicon into a per-domain folder, using filenames
  that include the method name.
- Compute per-method scores: a method receives 1 point for a domain if it
  fetched the highest-quality icon among all methods.
- Compute combined score: how many domains had at least one icon found.

Notes:
- This is a *test/benchmark utility*; it performs many network requests.
...
def get_all_domains(db_path: str) -> List[str]:
    ...
    # Same query as in the original test.
...
def _sniff_mime_from_bytes(data: bytes) -> Optional[str]:
    # Minimal sniffing; we also rely on HTTP headers.
...
def _parse_ico_biggest_size(data: bytes) -> Tuple[Optional[int], Optional[int]]:
    # ICO header: reserved(2) type(2) count(2), then count * 16 dir.
    try:
        if len(data) < 6:
            return None, None
...
def _parse_svg_size(data: bytes) -> Tuple[Optional[int], Optional[int]]:
    # SVG is vector; we treat as highest class. Size parsing is optional.
    return None, None


def determine_dimensions(mime: str, ext: str, data: bytes) -> Tuple[Optional[int], Optional[int]]:
    if ext == "svg" or mime.startswith("image/svg"):
        return _parse_svg_size(data)
...
def quality_key(c: IconCandidate) -> Tuple[int, int, int]:
    ...
    1) prefer SVG
    2) then by max dimension (estimated)
    3) then by byte size as a tiebreaker
    """
...
def save_candidate(base_dir: str, c: IconCandidate) -> str:
    dom_dir = os.path.join(base_dir, sanitize_domain(c.domain))
    os.makedirs(dom_dir, exist_ok=True)

    # Avoid collisions: include hash prefix.
    name = f"{c.method}__{c.max_side or 0}x{c.max_side or 0}__{c.sha256[:10]}.{c.ext}"
    path = os.path.join(dom_dir, name)
...
def fetch_google_s2(domain: str) -> List[Tuple[str, bytes, str]]:
    # Try multiple sizes; S2 supports sz.
...
def fetch_google_v2(domain: str) -> List[Tuple[str, bytes, str]]:
    # Try multiple sizes; v2 supports size.
    sizes = [32, 64, 128, 256]
    out: List[Tuple[str, bytes, str]] = []
    for sz in sizes:
        u = (
...
def fetch_iconhorse(domain: str) -> List[Tuple[str, bytes, str]]:
    u = f"https://icon.horse/icon/{domain}"
    data, headers, final_url = fetch_url_bytes(u, timeout=10)
    if not data:
        return []
    mime = headers.get("content-type") or _sniff_mime_from_bytes(data) or "application/octet-stream"
    return [(final_url or u, data, mime)]
...
def fetch_html_icons(domain: str) -> List[Tuple[str, bytes, str]]:
    ...
    icons = list(dict.fromkeys(parser.icons))  # de-dupe order stable
...
def fetch_manifest_icons(domain: str) -> List[Tuple[str, bytes, str]]:
    ...
    # Try all icons and keep all successes.
    out: List[Tuple[str, bytes, str]] = []
    for icon in icons:
...
def ensure_output_dir() -> str:
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_favicons_hires_output")


def per_domain_job(domain: str, output_dir: str, save_all: bool = True) -> Tuple[str, Dict[str, List[IconCandidate]]]:
    results: Dict[str, List[IconCandidate]] = {}
    for method, fn in METHODS:
        triples = fn(domain)
        candidates = build_candidates(domain, method, triples)
        results[method] = candidates

        if save_all:
            for c in candidates:
                save_candidate(output_dir, c)
...
def main() -> int:
    ...
    print(f"\nCollecting high-res favicons (workers=10), saving to: {output_dir}")
    start = time.time()

    per_domain: Dict[str, Dict[str, List[IconCandidate]]] = {}
    with ThreadPoolExecutor(max_workers=10) as ex:
        futures = [ex.submit(per_domain_job, d, output_dir, True) for d in domains]
        done = 0
        last_log = time.time()
        for fut in as_completed(futures):
            ...
            if now - last_log >= 2.0:
                ...
                print(f"Progress: {done}/{total} domains ({rate:.1f}/s)")
                last_log = now

    elapsed = time.time() - start

    # Determine winners per domain.
    method_points: Dict[str, int] = {m: 0 for m, _ in METHODS}
    method_found: Dict[str, int] = {m: 0 for m, _ in METHODS}
    domains_with_any = 0

    winners_per_domain: Dict[str, Dict[str, object]] = {}

    for d in domains:
        by_method = per_domain.get(d, {})
        all_candidates: List[IconCandidate] = []
        for m, cand in by_method.items():
            if cand:
                method_found[m] = method_found.get(m, 0) + 1
        ...
        best = max(all_candidates, key=quality_key)
        winners = [c for c in all_candidates if quality_key(c) == quality_key(best)]

        # Point to all methods that achieved the best quality for this domain.
        for c in winners:
            method_points[c.method] = method_points.get(c.method, 0) + 1

        winners_per_domain[d] = {
            "best": {
                "method": best.method,
                "url": best.url,
                "mime": best.mime,
                "ext": best.ext,
                "is_svg": best.is_svg,
                "max_side": best.max_side,
                "bytes": best.bytes_len,
                "sha256": best.sha256,
            },
            "tied_methods": sorted(list({c.method for c in winners})),
        }

    # Summary
    print("\n--- Summary ---")
    print(f"Elapsed: {elapsed:.2f}s")
    print(f"Domains with at least one icon: {domains_with_any}/{total} ({(domains_with_any/total*100.0 if total else 0.0):.1f}%)")
    print("\nPer-method: found any / score(best-quality points)")
    for m, _ in METHODS:
        print(f"- {m}: found={method_found.get(m, 0)}/{total}, points={method_points.get(m, 0)}")
    ...
    report = {
        ...
        "combined": {
            "domains_with_any_icon": domains_with_any,
            "coverage": (domains_with_any / total if total else 0.0),
        },
        "methods": {
            m: {
                "domains_with_any": method_found.get(m, 0),
                "best_quality_points": method_points.get(m, 0),
            }
            for m, _ in METHODS
        },
        "winners_per_domain": winners_per_domain,
    }

    report_path = os.path.join(output_dir, "hires_report.json")
